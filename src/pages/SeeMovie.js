...
class Movie extends React.Component {
  ...
  renderCastingMovie = (casting) => {
      ...
      return casting.cast.map((actor, index) => {
        ...
        return (
          <li key={`${actor.id}-${index}`} className="movie-details__actor cursorPointer">
            ...
            <p>{actor.name}</p>
            <p>
              <span role="img" aria-label="emoji cinéma">
                🎬
              </span>{" "}
              {actor.character}
            </p>
    ...
    return <p>Impossible d'afficher le casting de ce film</p>;
  };
  ...
  renderMovie = (movie) => {
    ...
    const actionBtn = this.state.isMovieInFav ? (
      <button onClick={this.deleteMoviefromFavAndRegenerateList} className="btn-action remove-from-favorites">
        Supprimer des favoris
      </button>
    ) : (
      <button onClick={this.addMovieInFavAndRegenerateList} className="btn-action add-to-favorites">
        Ajouter aux favoris
      </button>
    );
        ...
        <div className="layout-single__primary__infos">
          ...
          <div className="layout-single__left-main">
            ...
            {details.vote_average && (
              <p>
                <span role="img" aria-label="emoji étoile">
                  ⭐️
                </span>{" "}
            ...
            {budget && (
              <p>
                <span role="img" aria-label="emoji dollar">
                  💵
                </span>{" "}
                {budget}
              </p>
            ...
            {details.overview && (
              <p className="longText">
                <span role="img" aria-label="emoji stylo plume">
                  🖋
                </span>{" "}
                {details.overview}
              </p>
            ...
            <ul className="movie-details__genders" arial-label="genres">
              {details.genres.map((genre, index) => (
                <li key={`${genre}-${index}`} className="btn-action category disabledBtn">
                  {genre.name}
                </li>
              ))}
            </ul>
          ...
          <h2>Casting</h2>
          <ul className="movie-layout-single__right">{this.renderCastingMovie(casting)}</ul>
        </div>
      </div>
    );
  };
