  ...
  render() {
    const renderFavoritesMoviesSection = (movies) => {
      if (!this.state.indexedDbSupported) {
        return <p>Vous devriez utiliser un navigateur moderne pour pouvoir consulter vos films préférés</p>;
      }
    ...
    return (
      <section className="favorites-page">
        ...
        {!navigator.onLine && (
          <p>
            <span role="img" aria-label="émoji éclair">
              ⚡️
            </span>{" "}
            Pour pouvoir consulter le détail de vos films favoris merci d'activer votre connexion{" "}
            <span role="img" aria-label="émoji éclair">
              ⚡️
            </span>
          </p>
        ...
        {this.state.movies && this.state.movies.length === 0 && (
          <p className="no-favories">
            Aucun film n'est enregistré dans vos favoris{" "}
